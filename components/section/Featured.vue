...
<template>
  <section class="mx-auto max-w-[1180px] px-6 py-16 sm:px-10 sm:py-[72px]">
    <div v-reveal class="flex items-baseline justify-between flex-wrap gap-4 mb-8">
      <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <ProjectsCard v-for="(project, i) in projects" :key="project._path" v-reveal="i * 100" :project="project" :accent="projectAccent(i)" :subtitle="`${projectCategory(project).toLowerCase()}/ · ${projectStack(project)}`" subtitle-class="text-yellow" />
    </div>
  </section>
</template>
